...
program
  ...
  .option("-m, --metas <num>", "Number of meta items to create. Default: 10", 10)
  .option("-M, --media <num>", "Number of media items to create. Default: 10", 10)
  .option(
    "-e, --endpoint <url>",
...
(async () => {
  ...
  for (var i = 0; i < program.media; i++) {
    mediaType = `${faker.random.arrayElement(["audio", "document", "image", "video"])}`;
    inputObj = { input: {} };
    inputObj["input"][mediaType] = {
      ...
      authorId: faker.random.number({
        min: authors[0].author.id,
        max: authors[program.authors - 1].author.id,
      }),
      meta: JSON.stringify(metas[faker.random.number({ min: 0, max: program.metas })]),
    };
    promises.push(
      postgraphile.mutation[`create${mediaType.charAt(0).toUpperCase() + mediaType.slice(1)}`](
        inputObj,
        `{ ${mediaType} { id, authorId, meta }}`,
      ),
    );
  }
  // Complete all asynchronous mutations and get the results.
